...
#endif
    #pragma note noelle independent=1 shared="J, dN, dS, dW, dE, c, rows, cols, iN, iS, jW, jE" private="i, j, k, Jc, G2, L, num, den, qsqr"
		for (int i = 0 ; i < rows ; i++) {
            for (int j = 0; j < cols; j++) {
...
#endif
    #pragma independent=1 shared="J, c, rows, cols, lambda" private="i, j, k, D, cS, cN, cW, cE"
		for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
